...
figindex = n + 1;
constant = (2.*n + 1)/(2.*(3).^n);
integralIn = @(x) (1/6) .* besselk(0,sqrt((1+x)/2)) .* legendreP(n, x);
integralOut = integral(integralIn,-1,1);
outputTerm = (constant .* integralOut .* r.^n .* legendreP(n,cos(theta)));
outputMerged = outputMerged + outputTerm;

% Plotting Mesh Contributions (Omitting this for now, having issues with
% dimension of OutputTerm being 1x100)
